...
class Program
{
    static void Main()
    {
        ...
        using (PdfAnnotationEditor editor = new PdfAnnotationEditor())
        {
            ...
            var linkAnnotations = editor.ExtractAnnotations(3, 7, linkTypes);

            // Delete each extracted link annotation by its name
            foreach (Annotation annot in linkAnnotations)
            {
                // The Annotation class provides a Name property that uniquely identifies the annotation
        ...
        Console.WriteLine($"Link annotations on pages 3‑7 have been removed. Output saved to '{outputPath}'.");
    }
}